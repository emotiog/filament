...
int main(int argc, char** argv) {
    ...
    auto setup = [&app](Engine* engine, View* view, Scene* scene) {
        ...
        constexpr uint32_t SIZE_OF_VERTEX = 16;
        static_assert(sizeof(Vertex) == SIZE_OF_VERTEX, "Strange vertex size.");
        app.vb = VertexBuffer::Builder()
                .vertexCount(4)
                .bufferCount(1)
                .attribute(VertexAttribute::POSITION,   0, VertexBuffer::AttributeType::FLOAT2, 0, SIZE_OF_VERTEX)
                .attribute(VertexAttribute::UV0,        0, VertexBuffer::AttributeType::FLOAT2, 8, SIZE_OF_VERTEX)
                .build(*engine);
        app.vb->setBufferAt(*engine, 0,
                VertexBuffer::BufferDescriptor(QUAD_VERTICES, SIZE_OF_VERTEX * 4, nullptr));
        app.ib = IndexBuffer::Builder()
                .indexCount(6)
                .bufferType(IndexBuffer::IndexType::USHORT)
                .build(*engine);
        app.ib->setBuffer(*engine,
                IndexBuffer::BufferDescriptor(QUAD_INDICES, 2 * sizeof(uint16_t), nullptr));
